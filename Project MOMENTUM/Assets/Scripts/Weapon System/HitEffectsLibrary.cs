...
public class HitEffectsLibrary : MonoBehaviour
{
    ...
    public void SpawnParticles(RaycastHit hit, bool parent, float duration)
    {
        if (hit.collider.tag != "Player" && hit.collider.tag != "Enemy")
        {
            TranslateParticles(_bulletHolesPool, hit, Vector3.back, 20);
        }
    ...
    private void GetSurfaceParticles(RaycastHit hit)
    {
        switch (hit.collider.tag)
        {
            ...
            case "Player":
            case "Enemy":
                TranslateParticles(_bloodParticlesPool, hit, Vector3.forward, 2);
                break;
